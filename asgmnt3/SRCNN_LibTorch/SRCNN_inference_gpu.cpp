...
int main() {
	...
	if (torch::cuda::is_available()) {
		...
		std::cout << "Using GPU for calculation!" << "\n";
	} else {
		std::cout << "Using CPU for calculation!" << "\n";
	}
    network->to(device);
    ...
    for (auto& batch : *valid_data_loader) {
		// to skip the manual convolution
        batch_idx = 1; printf("manual convolution is been skipped!\n");
        break;

        // Get the images and labels, and move them to the device
        torch::Tensor images = batch.data.to(torch::Device(torch::kCPU));
        torch::Tensor labels = batch.target.to(torch::Device(torch::kCPU));
